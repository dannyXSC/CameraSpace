...
class RobomimicOpenPIRunner(BaseImageRunner):
    """
    ...
    def run(self):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                img_height = 84
                env_action = []
                for env_idx in range(len(self.env_fns)):
                    cur_obs = dict_apply(np_obs_dict, lambda x: x[env_idx][-1])
                    # print(cur_obs.keys())
                    # import pdb; pdb.set_trace()
                    
                    # c h w to h w c
                    img = cur_obs["agentview_image"].transpose(1, 2, 0)
                    wrist_img = cur_obs["robot0_eye_in_hand_image"].transpose(1, 2, 0)
                    
                    # img = np.ascontiguousarray(cur_img[::-1, ::-1])
                    # wrist_img = np.ascontiguousarray(cur_wrist_img[::-1, ::-1])
                    img = image_tools.convert_to_uint8(
                        image_tools.resize_with_pad(img, img_width, img_height)
                    )
                    wrist_img = image_tools.convert_to_uint8(
                        image_tools.resize_with_pad(wrist_img, img_width, img_height)
                    )
                
                    element = {
                        "observation/agentview_image": img,
                        "observation/robot0_eye_in_hand_image": wrist_img,
                        "observation/state": np.concatenate(
                            (
                                cur_obs["robot0_eef_pos"],
                                cur_obs["robot0_eef_quat"],
                                cur_obs["robot0_gripper_qpos"],
                            )
                        ),
                        "prompt": self.prompt,
                    }
                    ...
                    action_step = min(action_chunk.shape[0], self.n_action_steps)
                    env_action.append(action_chunk[:action_step])
