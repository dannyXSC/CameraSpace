...
from diffusion_policy.dataset.my_robomimic_dataset import get_camera_extrinsic_matrix
...
class RobomimicImageRunner(BaseImageRunner):
    ...
    def run(self, policy: BaseImagePolicy):
        tmp_env = EnvUtils.create_env_from_metadata(
            env_meta=self.env_meta, render=False, render_offscreen=False
        )
        camera_mat = get_camera_extrinsic_matrix(tmp_env.env.sim, "agentview")
        interface = RobosuiteInterface(tmp_env.env)
    ...
    def undo_transform_action(self, action, camera_mat, interface):
        ...
        camera_space_pose = PoseUtils.make_pose(pos, rot)
        world_pose = camera_space_pose @ camera_mat
        bs, hz = world_pose.shape[:2]
        # bs, hz -> bs*hz
        world_pose = world_pose.reshape(-1, 4, 4)
        acts = []
        for pose in world_pose:
            acts.append(interface.target_pose_to_action(pose, relative=False))
        acts = np.array(acts, dtype=np.float32)
        # bs*hz -> bs, hz
        acts = acts.reshape(bs, hz, -1)
        uaction = np.concatenate([acts, gripper], axis=-1)
