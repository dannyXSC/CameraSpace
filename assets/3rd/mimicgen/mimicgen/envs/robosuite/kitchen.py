...
class Kitchen_D0(KitchenEnv, SingleArmEnv_MG):
    ...
    def _load_model(self):
        ...
        stove_body = self.stove_object_1.get_obj(); stove_body.set("pos", array_to_string((0.23, 0.095, 0.02))); mujoco_arena.table_body.append(stove_body)

        self.button_object_1 = ButtonObject(
            name="Button1",
        )

        button_body = self.button_object_1.get_obj(); button_body.set("quat", array_to_string((0., 0., 0., 1.)));  button_body.set("pos", array_to_string((0.06, 0.10, 0.02))); mujoco_arena.table_body.append(button_body)

        self.serving_region = ServingRegionObject(
            name="ServingRegionRed"
        )
        serving_region_object = self.serving_region.get_obj(); serving_region_object.set("pos", array_to_string((0.345, -0.15, 0.003))); mujoco_arena.table_body.append(serving_region_object)
