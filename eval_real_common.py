...
@click.option("--target", "-t", required=True, help="Target hdf5 to inference")
...
def main(ckpt, target, steps_per_inference, frequency, device):
    ...
    try:
        ...
        def get_obs():
            ...
            robot_obs = torch.from_numpy(np.transpose(robot_obs, (2, 0, 1)))
            ...
            return robot_obs, eef_pos, eef_rot, torch.tensor([gripper])
        ...
        while True:
            ...
            obs["robot"] = torch.stack([prev_target_pose[0], cur_obs[0]], dim=0)
